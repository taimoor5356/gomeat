...
class CustomerLogic
{
    ...
    public static function create_wallet_transaction($user_id, $amount, $transaction_type, $referance)
    {
        ...
        if (in_array($transaction_type, ['add_fund_by_admin', 'add_fund', 'order_refund', 'loyalty_point', 'referrer'])) {
            ...
            if ($transaction_type == 'add_fund') {
                ...
                $check_loyalty_point_exchange_rate = (double) BusinessSetting::where('key', 'loyalty_point_exchange_rate')->first()->value;

                if($check_loyalty_point_exchange_rate == 0){
                    
                    $credit = (double)(($amount / 1)/100);
                }
                else{
                    $credit = (double)(($amount / BusinessSetting::where('key', 'loyalty_point_exchange_rate')->first()->value)/100);
                }
    ...
    public static function create_loyalty_point_transaction($user_id, $referance, $amount, $transaction_type)
    {
        ...
        if ($transaction_type == 'order_place') {
            $credit = ($amount * $settings['loyalty_point_item_purchase_point']) / 100;
            $credit = $credit * $settings['loyalty_point_exchange_rate'];
