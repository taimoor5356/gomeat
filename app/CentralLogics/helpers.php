...
class Helpers
{
    ...
    public static function store_data_formatting($data, $multi_data = false)
    {
        ...
        if ($multi_data == true) {
            foreach ($data as $item) {
                ...
                // $item['tax '] = BusinessSetting::where('key', 'service_fee')->first()->value;
    ...
    public static function currency_symbol($country = null)
    {
    ...
    public static function format_currency($value, $country = null)
    {
        $currency_symbol_position = BusinessSetting::where(['key' => 'currency_symbol_position'])->first()->value;

        return $currency_symbol_position == 'right' ? number_format($value, config('round_up_to_digit')) . ' ' . self::currency_symbol($country) : self::currency_symbol($country) . ' ' . number_format($value, config('round_up_to_digit'));
    }
    ...
    public static function send_one_signal_sms_to_customer_device($number, $data)
    {

        SMS_module::send_sms($number, $data);
        // $app_id = BusinessSetting::where(['key' => 'one_signal_app_id_customer'])->first()->value;
        // $api_key = BusinessSetting::where(['key' => 'one_signal_api_key_customer'])->first()->value;
        // $form_number = BusinessSetting::where(['key' => 'phone'])->first()->value;
        ...
        //         // "include_player_ids"=>[$oneSignalUserId->one_signal_id],
        //         // "included_segments"=>["Subscribed Users"],
        //         // "contents"=>json_decode('{"en":"English or Any Language Message"}'),
        //         "contents"=>json_decode('{"en":"'.$order_description.'"}'),
        //         'app_id' => $app_id,
        //         "name" => "GoMeat Services",
        //         "sms_from" => $form_number,
        //         // "sms_to" => "+923175222721",
        //         "include_phone_numbers" => [$number]
        //         // "include_phone_numbers" => ['+17732518701']
        //     ]);
        //     // dd($response->body());
    ...
    public static function send_one_signal_push_notif_to_customer_device($player_id, $data)
    {
        $app_id = BusinessSetting::where(['key' => 'one_signal_app_id_customer'])->first()->value;
        $api_key = BusinessSetting::where(['key' => 'one_signal_api_key_customer'])->first()->value;
        // $user = User::where('id',$user_id)->first();
        ...
        try{
            ...
            $response = Http::withHeaders([
                'Authorization' => 'Basic '.$api_key,
                // 'Authorization' => 'Basic ZTFjYWYxZmYtMTcyNy00NTJjLThhMmItZjNjNzJiOGRjODUz',
                'accept' => 'application/json',
                'content-type' => 'application/json',
            ...
            // )
            ->post('https://onesignal.com/api/v1/notifications', [
                // "include_player_ids"=>['35a31e6c-0037-4cd2-91ec-3a740cd0688f'],
                ...
                // 'title' => 'OrDER',
                // 'sound' => 'notification.wav'
                // 'app_id' => 'f514540c-e276-4c76-8fa1-cdbe84b82407'
            ]);
            // dd($response->body());
            // dd($data['title']); // order
            // dd($data['description']); // order
    ...
    public static function send_one_signal_push_notif_to_store_device($player_id, $data)
    {
        $app_id = BusinessSetting::where(['key' => 'one_signal_app_id_store'])->first()->value;
        $api_key = BusinessSetting::where(['key' => 'one_signal_api_key_store'])->first()->value;
        // $user = User::where('id',$user_id)->first();
    ...
    public static function send_one_signal_push_notif_to_driver_device($player_id, $data)
    {
        $app_id = BusinessSetting::where(['key' => 'one_signal_app_id_driver'])->first()->value;
        $api_key = BusinessSetting::where(['key' => 'one_signal_api_key_driver'])->first()->value;
        // $user = User::where('id',$user_id)->first();
    ...
    public static function order_status_update_image($status)
    {
        if ($status == 'pending') {
            $data = 'order-pending.png';
        } elseif ($status == 'confirmed') {
            $data = 'order-confirmed.png';
        } elseif ($status == 'processing') {
            $data = 'order-processing.png';
        } elseif ($status == 'picked_up') {
            $data = 'order-out-for-delivery.png';
        } elseif ($status == 'handover') {
            $data = 'order-handover.png';
        } elseif ($status == 'delivered') {
            $data = 'order-delivered.png';
        } elseif ($status == 'delivery_boy_delivered') {
            $data = 'order-delivered.png';
        } elseif ($status == 'accepted') {
            $data = 'order-confirmed.png';
        } elseif ($status == 'canceled') {
            $data = 'order-canceled.png';
        } elseif ($status == 'refunded') {
            $data = 'order-pending.png';
        } else {
            $data = 'order-confirmed.png';
    ...
    public static function send_order_notification($order)
    {
        ...
        try {
            ...
            $image = self::order_status_update_image($status);
            if ($value) {
                $data = [
                ...
                self::send_one_signal_push_notif_to_customer_device($user->one_signal_id, $data);
                if($order->user_id==='5090' || $order->user_id==='4074')
                {
                }
                else
                {

                    self::send_one_signal_sms_to_customer_device($user->phone, $data);
                }
            ...
            if($order->order_status == 'canceled' )
            {
                ...
                $user = Vendor::where('id',$order->store_id)->first();
                self::send_one_signal_push_notif_to_store_device($user->one_signal_id, $data);
                DB::table('user_notifications')->insert([
                    'data' => json_encode($data),
            ...
            if (in_array($order->order_status, ['processing', 'handover']) && $order->delivery_man) {
                ...
                $user = DeliveryMan::where('id',$order->delivery_man_id)->first();
                self::send_one_signal_push_notif_to_driver_device($user->one_signal_id, $data);
                DB::table('user_notifications')->insert([
                    'data' => json_encode($data),
