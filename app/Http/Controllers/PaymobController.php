...
class PaymobController extends Controller
{
    ...
    {
        try {
            $ord = Order::with(['details', 'customer'])->where(['id' => session('order_id')])->first();
            $config = Helpers::get_business_settings('paymob_accept');
            $integrationId = 143565;
            // cash_on_delivery, card_payment, jazz_cash, easy_paisa, stripe, paypal, wallet, crypto, gomt, 
            if (!is_null($requestData)) {
                $ord = Order::with(['details', 'customer'])->where(['id' => $requestData['order_id']])->first();
                if ($requestData['payment_method'] == 'jazz_cash') {
                    $integrationId = 143568;
                } else if ($requestData['payment_method'] == 'easy_paisa') {
                    $integrationId = 143571;
                }
            }
            $value = $ord->order_amount;
            $customer = $ord->customer;
            $billingData = [
                "first_name" => isset($customer) ? $customer->f_name : 'first_name',
                "last_name" => isset($customer) ? $customer->l_name : 'last_name',
                "email" => isset($customer) ? $customer->email : 'email@email.com',
                "apartment" => "apartment",
                "floor" => "floor",
