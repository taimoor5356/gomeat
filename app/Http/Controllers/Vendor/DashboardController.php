...
class DashboardController extends Controller
{
    public function dashboard(Request $request)
    {
        ...
        $store_earnings = Order::
        where('order_status','delivered')
        ->where('payment_status','paid')
        ->where(['store_id' => Helpers::get_vendor_id()])->select(
            DB::raw('IFNULL(sum(net_to_store),0) as earning'),
            DB::raw('IFNULL(sum(gm_commission),0) as commission'),
