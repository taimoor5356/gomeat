...
class PasswordResetController extends Controller
{
    public function reset_password_request(Request $request)
    {
        ...
        $customer = User::Where(['phone' => $request['phone']])->where(['login_medium'=>'custom_email'])->first();
        ...
        if (isset($customer)) {
            ...
            if($response == 'success')
            {
                ...
                return response()->json(['otp' => $token,'message' => translate('messages.otp_sent_successfull')], 200);
            }
            else
            {
        ...
        return response()->json(['errors' => [
            ['code' => 'not-found', 'message' => 'Phone number not associated to any user!']
        ]], 404);
    }
