...
class LoyaltyPointController extends Controller
{
    public function point_transfer(Request $request)
    {
        ...
        if($request->user()->loyalty_point <= 0 || $request->user()->loyalty_point < (int)BusinessSetting::where('key','loyalty_point_minimum_point')->first()->value) return response()->json(['errors' => [ ['code' => 'point', 'message' => trans('messages.insufficient_point')]]], 203);
        ...
        try
        {
            ...
            info($ex);
            return response()->json(['errors' => [ ['code' => 'customer_wallet', 'message' => $ex]]], 203);
        }
