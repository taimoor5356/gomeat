            ...
            <div class="row align-items-center mb-3">
                ...
                <div class="col-3">
                    ...
                    <select name="country_id" class="form-control js-select2-custom"
                            onchange="set_filter('{{url()->full()}}',this.value,'country_id')" title="{{translate('messages.select')}} Country">
                        <option value="" {{!request('country_id') ? 'selected':''}}>{{translate('messages.all')}} Countries</option>
                        @foreach (\App\Models\Country::get() as $country)
                        <option
                                value="{{$country->id}}" {{request('country_id') == $country->id?'selected':''}}>
                                {{$country['name']}}
                            </option>
                        @endforeach
                    </select>                        
            ...
            <div class="table-responsive datatable-custom">
                <table id="datatable"
                    ...
                    @foreach($orders as $key=>$order)
                        ...
                        <tr class="status-{{$order['order_status']}} class-all @if($order->store->country_id == '19') bg-green-color @endif">
                            <td class="text-black-color">
                                {{$key+$orders->firstItem()}}
                            </td>
                            <td class="table-column-pl-0 text-black-color">
                                <a class="text-black-color" href="{{route($parcel_order?'admin.parcel.order.details':'admin.order.details',['id'=>$order['id']])}}">{{$order['id']}}</a>
                            </td>
                            <td class="text-black-color">{{date('d M Y',strtotime($order['created_at']))}}</td>
                            <td class="text-black-color">
                                @if($order->customer)
                                    <a class="text-capitalize text-black-color"
                                       href="{{route('admin.customer.view',[$order['user_id']])}}">{{$order->customer['f_name'].' '.$order->customer['l_name']}}</a>
                                @else
                                    <label class="badge badge-danger text-black-color">{{translate('messages.invalid')}} {{translate('messages.customer')}} {{translate('messages.data')}}</label>
                                @endif
                            </td>
                            @if ($parcel_order)
                            ...
                            <td class="text-black-color">
                                ...
                                @elseif ($order->store)
                                    <label class="badge badge-soft-primary text-black-color"><a href="{{route('admin.vendor.view', $order->store_id)}}" alt="view store">{{Str::limit($order->store?$order->store->name:translate('messages.store deleted!'),20,'...')}}</a></label>
                                @else
                                    <label class="badge badge-soft-danger text-black-color">{{Str::limit(translate('messages.not_found'),20,'...')}}</label>
                                @endif
                            </td>
                            <td class="text-black-color">
                                @if($order->payment_status=='paid')
                                    <span class="badge badge-soft-success text-black-color">
                                      <span class="legend-indicator bg-success text-black-color"></span>{{translate('messages.paid')}}
                                    </span>
                                @else
                                    <span class="badge badge-soft-danger text-black-color">
                                      <span class="legend-indicator bg-danger text-black-color"></span>{{translate('messages.unpaid')}}
                                    </span>
                                @endif
                            </td>
                            <td class="text-black-color">@isset($order->store)@if(!empty($order->store->country->currency_symbol)){{$order->store->country->currency_symbol}}@else $ @endif @else $ @endisset {{$order['order_amount']}}</td>
                            <td class="text-capitalize text-black-color">
                                @if($order['order_status']=='pending')
                                    <span class="badge badge-soft-info ml-2 ml-sm-3 text-black-color">
                                      <span class="legend-indicator bg-info text-black-color"></span>{{translate('messages.pending')}}
                                    </span>
                                @elseif($order['order_status']=='confirmed')
                                    <span class="badge badge-soft-info ml-2 ml-sm-3 text-black-color">
                                      <span class="legend-indicator bg-info text-black-color"></span>{{translate('messages.confirmed')}}
                                    </span>
                                @elseif($order['order_status']=='processing')
                                    <span class="badge badge-soft-warning ml-2 ml-sm-3 text-black-color">
                                      <span class="legend-indicator bg-warning text-black-color"></span>{{translate('messages.processing')}}
                                    </span>
                                @elseif($order['order_status']=='picked_up')
                                    <span class="badge badge-soft-warning ml-2 ml-sm-3 text-black-color">
                                      <span class="legend-indicator bg-warning text-black-color"></span>{{translate('messages.out_for_delivery')}}
                                    </span>
                                @elseif($order['order_status']=='delivered')
                                    <span class="badge badge-soft-success ml-2 ml-sm-3 text-black-color">
                                      <span class="legend-indicator bg-success text-black-color"></span>{{translate('messages.delivered')}}
                                    </span>
                                @elseif($order['order_status']=='failed')
                                    <span class="badge badge-soft-danger ml-2 ml-sm-3 text-black-color">
                                      <span class="legend-indicator bg-danger text-black-color text-capitalize"></span>{{translate('messages.payment')}}  {{translate('messages.failed')}}
                                    </span>
                                @else
                                    <span class="badge badge-soft-danger ml-2 ml-sm-3 text-black-color">
                                      <span class="legend-indicator bg-danger text-black-color"></span>{{str_replace('_',' ',$order['order_status'])}}
                                    </span>
                                @endif
                            </td>
                            <td class="text-capitalize text-black-color">
                                @if($order['order_type']=='take_away')
                                    <span class="badge badge-soft-dark ml-2 ml-sm-3 text-black-color">
                                        <span class="legend-indicator bg-dark text-black-color"></span>{{translate('messages.take_away')}}
                                    </span>
                                @else
                                    <span class="badge badge-soft-success ml-2 ml-sm-3 text-black-color">
                                      <span class="legend-indicator bg-success text-black-color"></span>{{translate('messages.delivery')}}
                                    </span>
                                @endif
                            </td>
                            <td class="text-black-color">
                                <a class="btn btn-sm btn-white text-black-color"
