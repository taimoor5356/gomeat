...
<div class="content container-fluid">
        <div class="row" id="printableArea" style="font-family: emoji;">
            ...
            <div class="col-5">
                @if ($order->store)
                @php ($country = isset($order->store->country) ? $order->store->country->short_name : '')
                ...
                <table class="table table-bordered mt-3" style="width: 98%; color:#000000">
                    ...
                    @if ($order->order_type == 'parcel')
                        <tr>
                            ...
                            <td>{{\App\CentralLogics\Helpers::format_currency($order->delivery_charge, $country)}}</td>
                        </tr>
                    @else
                        @php($sub_total=0)
                        @php($total_tax=0)
                                ...
                                <tr>
                                    ...
                                    <td class="text-break">
                                            ...
                                            @foreach(json_decode($detail['variation'],true)[0] as $key1 =>$variation)
                                                @if ($key1 != 'stock')
                                                    <div class="font-size-sm text-body">
                                                        <span>{{$key1}} :  </span>
                                                        <span class="font-weight-bold">{{$key1=='price'?\App\CentralLogics\Helpers::format_currency($variation, $country):$variation}}</span>
                                                    </div>                                                
                                                @endif
                                        ...
                                        <div class="font-size-sm text-body">
                                            ...
                                            <span class="font-weight-bold">{{\App\CentralLogics\Helpers::format_currency($detail->price, $country)}}</span>
                                        </div>
                                        @endif
                                        ...
                                        @foreach(json_decode($detail['add_ons'],true) as $key2 =>$addon)
                                            ...
                                            <div class="font-size-sm text-body">
                                                ...
                                                <span class="font-weight-bold">
                                                    {{$addon['quantity']}} x {{\App\CentralLogics\Helpers::format_currency($addon['price'], $country)}}
                                                </span>
                                            </div>
                                    ...
                                    <td style="width: 28%">
                                        ...
                                        {{\App\CentralLogics\Helpers::format_currency($amount, $country)}}
                                    </td>
                                </tr>
                ...
                <div class="row justify-content-md-end mb-3" style="width: 97%">
                    <div class="col-md-7 col-lg-7">
                        <dl class="row text-right">
                            ...
                            <dd class="col-6">{{\App\CentralLogics\Helpers::format_currency($sub_total, $country)}}</dd>
                            <dt class="col-6">{{translate('addon_cost')}}:</dt>
                            <dd class="col-6">
                                {{\App\CentralLogics\Helpers::format_currency($add_ons_cost, $country)}}
                                <hr>
                            </dd>
                            <dt class="col-6">{{translate('messages.subtotal')}}:</dt>
                            <dd class="col-6">
                                {{\App\CentralLogics\Helpers::format_currency($sub_total+$add_ons_cost, $country)}}</dd>
                            <dt class="col-6">{{translate('messages.discount')}}:</dt>
                            <dd class="col-6">
                                - {{\App\CentralLogics\Helpers::format_currency($order['store_discount_amount'], $country)}}</dd>
                            <dt class="col-6">{{translate('messages.coupon_discount')}}:</dt>
                            <dd class="col-6">
                                - {{\App\CentralLogics\Helpers::format_currency($order['coupon_discount_amount'], $country)}}</dd>
                            <dt class="col-6">{{translate('messages.vat/tax')}}:</dt>
                            <dd class="col-6">+ {{\App\CentralLogics\Helpers::format_currency($order['total_tax_amount'], $country)}}</dd>
                            <dt class="col-6">{{ translate('messages.delivery_man_tips') }}:</dt>
                            <dd class="col-6">
                                @php($delivery_man_tips = $order['dm_tips'])
                                + {{ \App\CentralLogics\Helpers::format_currency($delivery_man_tips, $country) }}
                            </dd>
                            <dt class="col-6">{{translate('messages.delivery_charge')}}:</dt>
                            <dd class="col-6">
                                @php($del_c=$order['delivery_charge'])
                                {{\App\CentralLogics\Helpers::format_currency($del_c, $country)}}
                                <hr>
                            </dd> 
                            @else
                            <dt class="col-6">{{ translate('messages.delivery_man_tips') }}:</dt>
                            ...
                            <dd class="col-6" style="font-size: 20px">{{\App\CentralLogics\Helpers::format_currency($order->order_amount, $country)}}</dd>
                        </dl>
                    </div>
                </div>
                <span>---------------------------------------------------------------------------------</span>
